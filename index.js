...
async function obtenerClientes(conn) {
  const [rows] = await conn.query('SELECT * FROM cliente ORDER BY nombre, apellido_paterno');
  return rows;
}
...
async function obtenerCategorias(conn) {
  const [rows] = await conn.query('SELECT * FROM categoria ORDER BY nombre_categoria');
  return rows;
}
...
app.get('/', async (req, res) => {
  ...
  try {
    ...
    res.render('index', { clientes, productos, pedidos, categorias, mensaje: null });
  } catch (err) {
    console.error(err);
    ...
    switch (action) {
      ...
      case 'guardar_cliente': {
        await conn.query(
          ...
          [req.body.nombre, req.body.apellido_paterno, req.body.apellido_materno || '',
           req.body.telefono || '', req.body.correo_electronico || '', req.body.direccion || '']
        );
        break;
      }
      ...
      case 'actualizar_cliente': {
        await conn.query(
          ...
          [req.body.telefono || '', req.body.correo_electronico || '', req.body.direccion || '', req.body.id_cliente]
        );
        break;
      }
      ...
      case 'eliminar_cliente': {
        const [[{ total }]] = await conn.query(
          'SELECT COUNT(*) as total FROM pedido WHERE id_cliente=?', [req.body.id_cliente]
        );
        if (total > 0) break; // No eliminar si tiene pedidos
        await conn.query('DELETE FROM cliente WHERE id_cliente=?', [req.body.id_cliente]);
        break;
      }
      ...
      case 'guardar_producto': {
        await conn.query(
          ...
          [req.body.nombre, req.body.descripcion || '', req.body.id_categoria,
           req.body.precio_unitario, req.body.disponible ?? 1]
        );
        break;
      }
      ...
      case 'actualizar_producto': {
        await conn.query(
          ...
          [req.body.precio_unitario, req.body.disponible ?? 0, req.body.id_producto]
        );
        break;
      }
      ...
      case 'eliminar_producto': {
        const [[{ total }]] = await conn.query(
          'SELECT COUNT(*) as total FROM detalle_pedido WHERE id_producto=?', [req.body.id_producto]
        );
        if (total > 0) break;
        await conn.query('DELETE FROM producto WHERE id_producto=?', [req.body.id_producto]);
        break;
      }
      ...
      case 'guardar_pedido': {
        ...
        const [pedidoResult] = await conn.query(
          ...
          [req.body.id_cliente, req.body.fecha_entrega, req.body.hora_entrega,
           subtotal, anticipo, saldo, req.body.observaciones || '']
        );
        ...
        const [[cliente]] = await conn.query('SELECT direccion FROM cliente WHERE id_cliente=?', [req.body.id_cliente]);
        await conn.query(
      ...
      case 'editar_pedido': {
        ...
        const [[{ subtotal }]] = await conn.query('SELECT subtotal FROM pedido WHERE id_pedido=?', [id_pedido]);
        const anticipo = parseFloat(req.body.anticipo) || 0;
        const saldo = subtotal - anticipo;
        await conn.query(
          'UPDATE pedido SET fecha_entrega=?, hora_entrega=?, anticipo=?, saldo=?, observaciones=? WHERE id_pedido=?',
          [req.body.fecha_entrega, req.body.hora_entrega, anticipo, saldo, req.body.observaciones || '', id_pedido]
        );
        break;
      }

      case 'cambiar_estado': {
        await conn.query('UPDATE pedido SET estado=? WHERE id_pedido=?', [req.body.estado, req.body.id_pedido]);
        break;
      }
    }
...
app.post('/ajax/detalles-pedido', async (req, res) => {
  const conn = await pool.getConnection();
  try {
    const pedido = await obtenerPedidoCompleto(conn, parseInt(req.body.id_pedido));
    if (pedido) {
